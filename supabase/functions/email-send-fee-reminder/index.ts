// Email - Send Fee Reminder endpoint
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { studentIds, feeData } = await req.json();
    ...
    for (const studentId of studentIds) {
      ...
      if (students && students.length > 0) {
        ...
        const { subject, html } = emailTemplates.feeReminder(student, feeData);
    ...
    return successResponse({
      success: true,
      message: `Sent ${results.filter(r => r.success).length} fee reminders`,
      results
    });
  } catch (error: any) {
